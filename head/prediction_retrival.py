...
def prediction_retrival(connection_string, database_name, id_location, request_time, max_age=30):
  ...
  last_prediction = mongo_collection.find({"id_location": id_location}).sort("request_time", pymongo.DESCENDING).limit(1).next()
...
if __name__ == "__main__":
  ...
  last_prediction = prediction_retrival(connection_string, "output", 5439, datetime.now())
